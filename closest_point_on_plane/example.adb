...
--  procedure Update(Ticks : in Integer) is
  procedure Update is
    ...
    POINT_DELTA       : constant Float := 0.5;
    POINT_SMALL_DELTA : constant Float := 0.1;
  ...
  begin
    ...
    if Keys(SDL.Keysym.K_R) = True then
      ...
      Point := Vector3.Object'(2.0, 0.0, 0.0);
    ...
    if Keys(SDL.Keysym.K_N) = True and NPressed = False then
      ...
      if ShiftPressed = True then
        ...
        Point.X := Point.X - POINT_SMALL_DELTA;
          
      else
        
        Point.X := Point.X - POINT_DELTA;
    ...
    if Keys(SDL.Keysym.K_O) = True and OPressed = False then
      ...
      if ShiftPressed = True then
        ...
        Point.Y := Point.Y + POINT_SMALL_DELTA;
          
      else
        
        Point.Y := Point.Y + POINT_DELTA;
  ...
  procedure Draw is
    ...
    ProjectedPoint : Vector3.Object := GM.ClosestPoint(Point, TestPlane);
  ...
  begin
    ...
    GL.glBegin(GL.GL_POINTS);
      ...
      GL.glVertex3f(GL.GLfloat(Point.X), GL.GLfloat(Point.Y), GL.GLfloat(Point.Z));

      GL.glColor3f(1.0, 0.0, 0.0);
      GL.glVertex3f(GL.GLfloat(ProjectedPoint.X), GL.GLfloat(ProjectedPoint.Y), GL.GLfloat(ProjectedPoint.Z));
